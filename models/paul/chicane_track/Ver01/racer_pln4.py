...
def race(model_path, host, name):
    ...
    AI_LAUNCH_DURATION = 1.0  #2.0  #2.7 #3.0            # the ai will output throttle for this many seconds
    AI_LAUNCH_THROTTLE = 0.9  #1.7 #1.8 
    AI_LAUNCH_STEERING = 0.00
    
    maxcount = int(AI_LAUNCH_DURATION/0.2) #2.7s 20Hz
    print("maxcount: ", maxcount)
    AI_START = True
    try:
        if AI_START:
            for i in range(maxcount): 
                client.send_controls(AI_LAUNCH_STEERING, AI_LAUNCH_THROTTLE)
                #client.send_controls(0.0, 0.8)
                time.sleep(0.2)
